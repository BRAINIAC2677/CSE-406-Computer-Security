...
def ecdh_key_exchange(_socket: socket, _security_level) -> int:
    ecdh = ECDHCipher.ECDHCipher(_security_level)
    curve = ecdh.get_parameters()
    # send nist_staqndard_curve object to Bob
    _socket.send(str(curve).encode())
    # receive Bob's public key
    bob_public_key = _socket.recv(1024).decode()
    #convert Bob's public key to Point object
    bob_public_key = ECDHCipher.Point(int(bob_public_key.split(',')[0][1:]), int(bob_public_key.split(',')[1][:-1]))
